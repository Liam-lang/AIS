...
def main(argv):
    if argv:
        try:
            opts, args = getopt.getopt(argv, "hvi:", ["db_name="])
        ...
        for opt, arg in opts:
            if opt == '-v':
                ...
                print "Verbose value is %s, the var_dbname is %s" % (
                    str(verbose), str(var_dbname))
    else:
        usage()
        sys.exit(2)
...
if __name__ == '__main__':
    dbname = main(sys.argv[1:])
    engine = model.connectDB(dbname)
